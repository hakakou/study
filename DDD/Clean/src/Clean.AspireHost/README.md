...
1. Set `Clean.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../Clean.Infrastructure/Clean.Infrastructure.csproj -s Clean.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../Clean.Infrastructure/Clean.Infrastructure.csproj -s Clean.Web.csproj
```
